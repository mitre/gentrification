...
pov <- reactive({
  start_end() %>%
    filter(Measure == "Percent in Poverty") %>%
...
start_end_labelled <- reactive({
  start_end() %>%
    ...
    add_gentrification_label_full("Percent in Poverty",
...
measures <- reactive({
  start_end_labelled() %>%
    mutate(`Positive Indicator` = rowSums(. == "Positive Indicator"),
           `Not Eligible` = rowSums(. == "Not Eligible"),
           `Still Susceptible` = rowSums(. == "Still Susceptible")) 
})
## Add three way color 
...
```{r color-define, echo = FALSE}

# test <- tibble(R = c(0, 1, 1, 2, 3, 0, 0, 2),
#                B = c(1, 0, 1, 0, 0, 0, 3, 1),
#                W = c(2, 2, 1, 1, 0, 3, 0, 0)) %>%
#   mutate(total = R + B + W) %>%
#   mutate(ramp = list(colorRampPalette(c("#0C6291", "#C05746"))(5)))
#   mutate(color = case_when(W == 3 ~ gent_colors[1],
#                            R == 3 ~ gent_colors[3],
#                            B == 3 ~ gent_colors[2],
#                            W == 0 & B == 1 ~ ramp[1],
#                            W == 0 & B == 2 ~ ramp[2]
...
# 
# 
# gent_ramp <- colorRampPalette(c(gent_colors[2:3]))(3)
# show_col(gent_colors)
# colorRampPalette(c(gent_ramp))
```
